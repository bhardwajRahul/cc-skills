...
- **Timeout**: `5` in `hooks.json`. That field is **seconds**, not milliseconds — the shipped binary computes `hook.timeout * 1000` at **15 textual occurrences across 14 distinct call sites** (two of the fifteen are the same statement — the SessionEnd max-fold `if(p.timeout&&p.timeout*1000>n)n=p.timeout*1000` — which is why a raw `grep -c` says 15 and a reading of the code says 14), and its embedded zod schema describes the field as "Timeout in seconds for this specific command" (re-verified against 2.1.258/259/260/261, identical in all four). Count with `grep -oa 'timeout\*1000'`, not `timeout \* 1000`: the three **spaced** matches are not hook code at all but Bun's embedded SQL client, a connection-pool `close({timeout})` option, so any figure of 18 is counting those by mistake. A four-digit value here would park the interactive question dialog for over an hour, and the timeout is the only backstop against a hang because the hook's top-level `catch` only covers a throw.
