...
describe("TOOL_SCHEMAS", () => {
  it("has schemas for 9 built-in tools", () => {
    // 10 -> 9: AskUserQuestion's schema was removed on 2026-09-24. Rewriting that tool's input
    // suppresses its dialog (see allowWithInput), so there is nothing a schema could safely enable.
  ...
  it("does NOT have schemas for UI tools", () => {
    // AskUserQuestion in particular must stay unmodelled: for it, updatedInput carries the user's
    // answers, so a hook mutation suppresses the dialog and returns "did not answer".
...
describe("validateToolInput", () => {
  ...
  it("returns invalid for AskUserQuestion, which has no schema", () => {
