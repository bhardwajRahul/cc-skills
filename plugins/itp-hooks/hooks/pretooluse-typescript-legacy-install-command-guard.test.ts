...
describe("pretooluse-typescript-legacy-install-command-guard", () => {
  describe("Legacy TypeScript versions (DENIED)", () => {
    it("denies `npm i -D typescript@5.9.3`", async () => {
      ...
      expect(result.decision === "deny" && result.reason).toContain("typescript@5.9.3");
    });
    ...
    it("denies `bun add -d typescript@^6`", async () => {
      ...
      expect(result.decision).toBe("deny");
      expect(result.decision === "deny" && result.reason).toContain("typescript@^6");
    });
    ...
    it("denies `pnpm add typescript@~6.0`", async () => {
      ...
      expect(result.decision).toBe("deny");
      expect(result.decision === "deny" && result.reason).toContain("typescript@~6.0");
    });
    ...
    it("denies `yarn add typescript@6.1.2`", async () => {
      ...
      expect(result.decision).toBe("deny");
      expect(result.decision === "deny" && result.reason).toContain("typescript@6.1.2");
    });
    ...
    it("denies `npm i -D @typescript/native-preview@7.0.0-dev`", async () => {
      ...
      expect(result.decision).toBe("deny");
      expect(result.decision === "deny" && result.reason).toContain("@typescript/native-preview");
    });
  });
  ...
  describe("Denial message quality", () => {
    ...
    it("includes the SSoT path in denial reason", async () => {
      ...
      expect(result.decision === "deny" && result.reason).toContain("~/.claude/typescript-latest-CLAUDE.md");
    });
    ...
    it("includes the compat alias fix in denial reason", async () => {
      ...
      expect(result.decision === "deny" && result.reason).toContain("@typescript/typescript6");
    });
