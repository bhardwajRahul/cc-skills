...
import { classifyLeakageTaxonomyForPostToolUseOrchestrator } from "./posttooluse-leakage-taxonomy-reminder.ts";
...
const POSTTOOLUSE_EDIT_TIME_ORCHESTRATOR_SUBHOOK_REGISTRY: PostToolUseSubhookRegistryEntry[] = [
  {
    name: "leakage-taxonomy-reminder",
    timeoutMs: 2000,
    classify: classifyLeakageTaxonomyForPostToolUseOrchestrator,
    description:
      "After a Write/Edit/MultiEdit of a TEXT file, injects the five-category temporal-leakage taxonomy ONCE PER SESSION when the NEW content contains language that ADJUDICATES leakage. Guards the OVER-RULING-OUT failure mode measured in the xaubot audit: agents flatten every temporal irregularity to \"leakage\", every \"leakage\" to \"reject\", and throw away a valid result. The card carries the one correction agents get wrong most — CAT-2 (train-time-only, e.g. a scaler fitted on the training fold) is NOT a leak, is worth a measured +0.0014 (clean_reg arm) to +0.0042 (cat2_only arm) AUC, and IS removed by walk-forward, whereas CAT-3 survives an 8-fold rolling WF at leaky WFE 0.991 and an embargo swept to 5,000 bars — plus UNKNOWN-is-not-a-finding, prefix invariance as the only decisive test, and the mandatory-availability-exclusion vs optional-dependence-gap split. DETECTION IS TWO-TIERED ON BOTH SIDES, because precision and the once-per-session gate are ONE axis: since the card fires at most once, every false positive is ALSO a false negative that spends the session's single shot before the real verdict is written. Leak terms split into DOMAIN (look-ahead/lookahead/data-snooping/non-causal/acausal/contaminat*/leakage/CAT-[123]/prefix-invariance/qualified compounds like `data leak`) and GENERIC (bare leak*); verdict terms split into DECISIVE (falsif*/refut*/reject*/condemn*/manufactur*/spurious/overstat*/`no edge`/`not genuine`/rule out — read OFF the corpus, not guessed) and WEAK (fail*/invalid family/discard*/dismiss*/fatal*/artifact/collaps*/inflat*/`coin flip`). FIRING RULE: DOMAIN ∧ (DECISIVE ∨ WEAK), or GENERIC ∧ DECISIVE — a weak word never fires on a bare leak. Proximity is tiered too and calibrated, not guessed: 200 chars for DOMAIN+DECISIVE, 100 for GENERIC, 80 for WEAK (every genuine corpus pair lands at d<=48; the surviving false positives clustered at d=79..141). A ±80-char SENSE WINDOW discards a leak hit whose neighbourhood names a non-temporal sense — memory/heap/goroutine/descriptor, relu/torch/nn., one-token/parser/lexer, filtfilt/zero-phase, water/gasket, `the press`/memo, gitleaks/credential. Two vocabulary corrections are load-bearing and must not be reverted: `leaky` is NOT a verdict word (as one it collapsed the conjunction into \"mentions leak twice\" — 15/15 sentence-level fires over 1,448 corpus sentences were that artifact and none was an adjudication), and the invalid family is an explicit alternation because `invalid\\w*` matched \"cache invalidation\", the commonest real-source false positive. MEASURED after these changes: 11/11 leak-bearing xaubot corpus documents fire (VERDICT.json and DEEP-DIVE-VERDICT.json included, both silent before), 0/10 naturalistic non-temporal negatives, 0/60 leak-mentioning real repo files, 6/1198 (0.5%) on a random real-file sample of which 5 are genuine research adjudications. NON-BLOCKING BY CONSTRUCTION: a deny here would commit the very error it guards, so it only ever returns noop or additional_context. MultiEdit fragments are scanned SEPARATELY so proximity cannot be manufactured across unrelated edits; detection reads the NEW content only. LEAK-TAXONOMY-OK suppresses from the edited fragment OR — genuinely, via a real post-match file read — anywhere in the post-edit file, which is what stops an ordinary Edit of the marker-bearing doctrine spoke from firing the hook on its own doctrine. session_id is sanitised to one path component before it reaches the shared gate helper, which interpolates it straight into /tmp. Message pinned in BOTH units by test (<=900 chars and <=960 UTF-8 bytes; 896/907 today) because it is paid on every fire. Extension allowlist plus a NUL-byte prefix sniff keep binary payloads out; temp-scratch exempt via the iter-124 helper so a throwaway file cannot burn the gate, which is claimed only AFTER a match AND after whole-file suppression is ruled out. Single-pass regex scan, no subprocess, and no file read on the common path — registry position FIRST (cheapest entry: O(1) extension pre-filter pre-empts everything else). Algorithm in classifyTemporalLeakageAdjudicationVerdictLanguageOncePerSessionForPostToolUseOrchestrator (alias classifyLeakageTaxonomyForPostToolUseOrchestrator); doctrine SSoT ~/.claude/leakage-taxonomy-CLAUDE.md (operator directive 2026-08-22).",
  },
  {
    name: "ty-type-check",
    timeoutMs: 5000,
