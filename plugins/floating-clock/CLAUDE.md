...
| **Linked frameworks**   | `Cocoa`, `Foundation`, `AppKit`, `CoreFoundation`, `libobjc`, `libSystem` (main binary), plus `QuartzCore`, `CoreLocation`, `CoreAudio`, `AVFoundation`, `IOBluetooth`, `CoreText`, `SystemConfiguration`, `CoreWLAN` and `Metal` for the overlay indicators (`Metal` 2026-09-19: the brightness rail presents a 1×1 EDR frame). Plus `ImageIO`, `UniformTypeIdentifiers`, `CoreServices` (build-time `gen-icon` helper only). One PRIVATE framework is `dlopen`'d and never linked — `DisplayServices`, so a missing symbol degrades the brightness rail instead of breaking the build. All system, not third-party.                                                                                        |
...
## Brightness rail (0–140%) — [docs/brightness-rail.md](./docs/brightness-rail.md)

Top of the indicator stack (2026-09-19). One continuous control: 0–100% drives the ordinary macOS brightness value via a `dlopen`'d DisplayServices SPI; above 100% it unlocks EDR headroom the slider cannot reach, using a 1×1 Metal EDR trigger plus a gamma ramp scaled above 1.0. Gamma reverts on process death including SIGKILL. Collapses to a plain 0–100 slider on any Mac without extra headroom. Defaults: `BrightnessBarEnabled`, `BrightnessBarStep`. Mechanism, safety guards, attack-test results, licensing and cost: the spoke above.
