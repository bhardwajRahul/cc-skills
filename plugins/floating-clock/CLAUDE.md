...
| **Linked frameworks**   | `Cocoa`, `Foundation`, `AppKit`, `CoreFoundation`, `libobjc`, `libSystem` (main binary), plus `QuartzCore`, `CoreLocation`, `CoreAudio`, `AVFoundation`, `IOBluetooth`, `CoreText`, `SystemConfiguration` and `CoreWLAN` for the overlay indicators. Plus `ImageIO`, `UniformTypeIdentifiers`, `CoreServices` (build-time `gen-icon` helper only — not linked into the running app). All framework paths are system, not third-party.                                                                                  |
...
| **Subprocesses**        | Exactly one, and only on demand — `/usr/sbin/networksetup`, used by the network bar to list network services and to reorder them when you pick a different route. NEVER spawned from the 1 Hz tick (that would wreck the idle-CPU budget); the service list is cached and refetched only on a menu open, a switch, or an unknown primary interface. Measured idle: 0.3 % CPU, zero child processes.                                                                                                                    |
| **System mutations**    | The network bar's switch action changes SYSTEM-WIDE network service order via `networksetup -ordernetworkservices`, which moves the default route for every app on the machine, not just this one. It is the only system-wide change this plugin makes, it happens only on explicit user action (click or menu pick), and it is reversible by picking the previous service. On accounts without admin rights macOS may refuse; the failure surfaces as a transient ✗ in the bar rather than being swallowed.           |
