...
| amonic task wrapper (retired with mise; amonic now runs `moon run root:minimax-check-upgrade`) | (not shipped)                | SKIPPED    | 41   | Repo-specific glue stays in the consuming repo; the logic stays in the plugin. Consumers invoke `scripts/minimax-check-upgrade` directly or wrap it in their own task runner — see `references/api-patterns/model-upgrade-detection.md` "Wiring it into a consuming repo". |
...
PARTIAL:                         0
SKIPPED:                         3   (CLAUDE.md, LOOP_CONTRACT.md, task wrapper)
STUB:                            0
