...
- **2026-07-10 — drafts hard-wrapped at recipient (premature line wrapping).**
  - _Trigger_: sent drafts (project-a correspondence to the client) reflowed to short, fixed-width lines in the recipient's client. Cause: `buildRawMessage` set `Content-Type: text/plain; charset=utf-8` with no `format=flowed`, so long author paragraphs are treated as fixed lines and every client hard-wraps them at its own width.
