...
**Real-world example**: `client-career-opportunities` had a `successCmd` that ran quarto render + wrangler deploy + curl verify — all of which `release:full` already handled via `depends_post = ["publish:verify"]`. The `${CLOUDFLARE_API_TOKEN:-}` crashed lodash, requiring manual deploys after every release. Fix: delete the entire `successCmd` block ([commit 2a08ecd](https://github.com/terrylica/client-career-opportunities/commit/2a08ecd)).
