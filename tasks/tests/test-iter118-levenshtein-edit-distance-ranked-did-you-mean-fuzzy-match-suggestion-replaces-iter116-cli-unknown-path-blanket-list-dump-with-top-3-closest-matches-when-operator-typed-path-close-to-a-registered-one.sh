...
set +e
CLI_TYPO_OUTPUT=$(bash "$ITER116_OPERATOR_FACING_TASK_ABSOLUTE_PATH" "$SYNTHETIC_TYPO_OF_KNOWN_RUNTIME_HOOK_CONSUMER_PATH" 2>&1)
CLI_TYPO_EXIT_CODE=$?
set -e
...
set +e
CLI_UNRELATED_OUTPUT=$(bash "$ITER116_OPERATOR_FACING_TASK_ABSOLUTE_PATH" "$COMPLETELY_UNRELATED_PATH_GUARANTEED_TO_BE_FAR_FROM_EVERY_REGISTERED_PATH" 2>&1)
...
set +e
CLI_HAPPY_PATH_OUTPUT=$(bash "$ITER116_OPERATOR_FACING_TASK_ABSOLUTE_PATH" "$KNOWN_RUNTIME_HOOK_CONSUMER_PATH_FOR_FUZZY_PROBE" 2>&1)
