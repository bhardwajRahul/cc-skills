...
iter149_assert_substring_present_in_file \
    ...
    "$ITER149_RELEASE_FULL_TASK_ABSOLUTE_PATH" \
    "ITER-149"
...
ITER149_TOTAL_ASSERTIONS_EVALUATED=$((ITER149_TOTAL_ASSERTIONS_EVALUATED + 1))
if bash -n "$ITER149_RELEASE_FULL_TASK_ABSOLUTE_PATH" 2>/dev/null; then
...
if command -v shellcheck >/dev/null 2>&1; then
    if shellcheck "$ITER149_RELEASE_FULL_TASK_ABSOLUTE_PATH" >/dev/null 2>&1; then
