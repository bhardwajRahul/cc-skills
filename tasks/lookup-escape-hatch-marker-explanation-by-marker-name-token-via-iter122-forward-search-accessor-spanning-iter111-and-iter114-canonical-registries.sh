...
print_usage_and_exit_one() {
    echo "Usage: bash tasks/$(basename "$0") [--json] <marker-name-token>"
    echo ""
    ...
    echo ""
    echo "Examples:"
    echo "  bash tasks/lookup-escape-hatch-marker-explanation-by-marker-name-token-via-iter122-forward-search-accessor-spanning-iter111-and-iter114-canonical-registries.sh \\"
    echo "      FILE-SIZE-OK"
    echo ""
    echo "  # Case-insensitive fallback:"
    echo "  bash tasks/$(basename "$0") file-size-ok"
    echo ""
    echo "  # Substring search — finds CARGO-TTY-SKIP + CARGO-TTY-WRAP:"
    echo "  bash tasks/$(basename "$0") TTY"
    echo ""
    echo "  # JSON output piped to jq:"
    echo "  bash tasks/$(basename "$0") --json FILE-SIZE-OK | jq '.matchingMarkers[0].consumerHookSourceFileRelativePath'"
    echo ""
    echo "Exit codes:"
