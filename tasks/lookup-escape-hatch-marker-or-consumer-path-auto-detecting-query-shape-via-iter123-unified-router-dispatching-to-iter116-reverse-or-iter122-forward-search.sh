...
print_usage_and_exit_one() {
    echo "Usage: bash tasks/$(basename "$0") [--json] [--direction=forward|reverse|auto] <query>"
    echo ""
    ...
    echo "  bash tasks/$(basename "$0") plugins/itp-hooks/hooks/pretooluse-file-size-guard.ts"
    echo ""
    echo "  # Auto-detect: ambiguous basename -> try forward first, fall back to reverse"
    echo "  bash tasks/$(basename "$0") file-size-guard"
    echo ""
    echo "  # Explicit direction override:"
    echo "  bash tasks/$(basename "$0") --direction=reverse file-size-guard"
    echo ""
    echo "Exit codes:"
