...
function renderTableOfContentsForOperatorQuickNavigationAcrossAllRegisteredMarkersGroupedByLifecycleLayer(
  ...
  tableOfContentsLines.push(
    \`**Audit-task markers** (\${sortedAuditTaskMarkerTokens.length}; consumed by \\\`tasks/\\\` audit scripts once per release-preflight):\`,
  );
  tableOfContentsLines.push("");
...
function renderCompleteOperatorFacingMarkdownReferenceDocument(): string {
  ...
  const auditTaskCatalogPreamble = [
    \`## Audit-task marker catalog (\${sortedAuditTaskMarkerTokens.length} registered markers consumed by tasks/ release-preflight audit scripts)\`,
    "",
